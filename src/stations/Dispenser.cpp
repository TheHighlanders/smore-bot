...
void Dispenser::selfTest() {
    logUpdate("%s: capture solenoid", name().c_str());
    writeChannel(m_p1, 1, m_config.capture);
    delay(kPulseMs);
    writeChannel(m_p1, 0, m_config.capture);

    logUpdate("%s: servo pin %u to %u", name().c_str(), m_config.servoPin,
              m_config.servoDispense);
    analogWrite(m_config.servoPin, m_config.servoDispense);
    delay(kPulseMs);
    analogWrite(m_config.servoPin, m_config.servoParked);
}
