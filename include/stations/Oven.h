...
class Oven : public Station{
    public:
        ...
        Oven(std::string name, P1AM& p1, OvenHWConfig hwConfig, Machine* machine) : Station(name), hardware(p1), config(hwConfig), atTemp(false), trayInside(false), tempSerial(name+"temp", PERSISTENT), exitSerial(name+"exit", EPHEMERAL) {
            m_machine = machine;
